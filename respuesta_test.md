...
3. La infraestructura(Servidores)
    ...
    Reciben peticiones de usuarios y les devuelven pag web,archivos generadas por app.

    El php-fpm tiene mejor gestion de memoria y rendimiento bajo. Sirve archivos estaticos y este gestiona los scripts de php y permite pools sirven para cuando se manejan varios sitios web a la vez,mientras que el CGI crean procesos del SO que conlleva un coste altisimo.

    Laravel sirve html, gestion de ciclo de vda de app, los pools de conexion con BD, transaccion distribuidas, iyeccion de dependencias y seguridad declarativa.

4. Evaluacion de Herramientas y Framework
    
    Se elige php y laravel 12 tiene mapeo objeto-relacional que permite trabajar con la BD usando obj php sin escribir SQL, utiliza una herrmaienta de linea de comandos para generar codigo, ejecutar migraciones y tareas de mantenimiento, tiene el Middleware que es una capa de seguridad que autentica, loggea y valida peticiones y Migrations un control de versiones para esquema de la BD.